...
export default function MIT() {
  ...
  const blueprintImages: LBImage[] = [
    { src: GROUND_FLOOR_PLAN, alt: "Ground Floor Plan — MIT", category: "Blueprint" },
    { src: FIRST_FLOOR_PLAN, alt: "First Floor Plan — MIT", category: "Blueprint" },
    { src: SECOND_FLOOR_PLAN, alt: "Second Floor Plan — MIT", category: "Blueprint" },
  ];

  const galleryLightboxImages: LBImage[] = GALLERY_IMAGES.map((src, i) => ({
    src,
    alt: `Gallery Image ${i + 1} — MIT`,
    category: "Gallery",
  }));
  ...
  return (
    <div className="relative bg-background-50">
      <PageMeta
        title="MIT | Unbuilt Project | Sanctuary Architects & Designers"
        description="Explore MIT Architecture Faculty, an unbuilt educational project by Sanctuary Architects & Designers at Manipal Institute of Technology, designed with climatic considerations, local materials, and thoughtful spatial planning."
        keywords="MIT Architecture Faculty, Manipal Institute of Technology, unbuilt educational project, climate-responsive architecture, Sanctuary Architects"
        canonicalPath="/projects/unbuilt/mit"
      />
      <Navbar />
        ...
        <section className="relative w-full min-h-[70vh] md:min-h-[85vh] flex items-end pb-14 md:pb-20 overflow-hidden">
          ...
          <div className="relative z-10 w-full px-6 md:px-10 lg:px-14">
            <nav className="flex items-center gap-2 mb-6 text-xs font-body tracking-[0.04em] flex-wrap">
              ...
              <i className="ri-arrow-right-s-line text-background-200/40 text-[10px]" />
              <span className="text-background-50">MIT</span>
            </nav>
                ...
                {blueprintImages.map((bp, idx) => (
                  ...
                  >
                    ...
                    <div className="px-5 pb-5 pt-1 flex items-center justify-between">
                      <span className="font-heading text-base md:text-lg font-light text-foreground-950">{bp.alt.replace(" — MIT", "")}</span>
